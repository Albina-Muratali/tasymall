    ...
    if request.method == "POST":
        if request.POST.get('action') == 'update_profile':
            ...
            try:
                ...
                messages.error(request, 'Error updating phone number: {}'.format(e))
            return redirect(reverse('customer:profile'))
...
def payment_method_page(request):
    ...
    if stripe_payment_methods and len(stripe_payment_methods.data) > 0:
        ...
        if card_details:
            current_customer.stripe_card_last4 = card_details.last4
        else:
            current_customer.stripe_card_last4 = ""
        current_customer.save()
    else:
...
def create_job_page(request):
    ...
    step1_form = forms.JobCreateStep1Form(request.POST or None, request.FILES or None, instance=creating_job)
    step2_form = forms.JobCreateStep2Form(request.POST or None, instance=creating_job)
    step3_form = forms.JobCreateStep3Form(request.POST or None, instance=creating_job)

    if request.method == "POST":
        if request.POST.get('step') == '1' and step1_form.is_valid():
            creating_job = step1_form.save(commit=False)
            creating_job.customer = current_customer
            creating_job.save()
            return redirect(reverse('customer:create_job'))

        elif request.POST.get('step') == '2' and step2_form.is_valid():
            step2_form.save()
            return redirect(reverse('customer:create_job'))
            ...
            if step3_form.is_valid():
                ...
                try:
                    ...
                    result = r.json()
                    print(result)  # Р›РѕРіРёСЂРѕРІР°РЅРёРµ РѕС‚РІРµС‚Р° РѕС‚ Google Maps API
                    if result['rows'][0]['elements'][0]['status'] == 'ZERO_RESULTS':
                        messages.error(request, "No results found for this route.")
                        return redirect(reverse('customer:create_job'))
                    ...
                    print(e)
                    messages.error(request, "Unfortunately, we do not support shipping for this route.")
                    return redirect(reverse('customer:create_job'))
